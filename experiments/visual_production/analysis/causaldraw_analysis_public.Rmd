...
This is analysis notebook for *causaldraw* (production of visual explanations and depictions)
...
### split by condition
```{r}
stroke_dat$toy_id <- str_c(stroke_dat$toy_type, "_", stroke_dat$toy_variant)
stroke_dat$sketchID <- str_c(stroke_dat$gameID, ".", stroke_dat$toy_id)
...
#extraxct highest value within sketchID group
strokes_dep <- aggregate(currStrokeNum ~ sketchID, data = final_data_dep, max)
mean(strokes_dep$currStrokeNum)
...
#extraxct highest value within sketchID group
strokes_exp <- aggregate(currStrokeNum ~ sketchID, data = final_data_exp, max)
mean(strokes_exp$currStrokeNum)
```
