...
```{r}
## SANDBOX: DO NOT KEEP OR COMMIT
final_sketch %>% 
  filter(totalStrokes < (mean(.$totalStrokes) + 3*sd(.$totalStrokes))) %>% 
  ggplot(aes(x=totalStrokes, y=corrAns)) +
  geom_point(aes(color = condition)) +
  geom_smooth(method=lm, aes(color = factor(condition))) +
  geom_smooth(method=lm) +
...
overall_strokes <- glmer(corrAns ~ totalStrokes + (1|sketchID),  family="binomial", data = final_data_filtered, nAGQ=0)
summary(overall_strokes)

overall_strokes_by_condition <- glmer(corrAns ~ totalStrokes*condition + (1|sketchID),  family="binomial", data = final_data_filtered, nAGQ=0)
# summary(overall_strokes_by_condition)
# anova(overall_strokes, overall_strokes_by_condition)
```
