...
ggsave('../results/plots/intervention_corrAns_checking.pdf', height = 8, width = 3,  units = 'cm', useDingbats = F)
```
...
## follow-up analysis
here we compare how explanations with MORE structural information vs. LESS structural information do on this part-discrimination task. 
```{r}
## median split data by more or less structural details for explanatory drawings
high_structural_exp <- subset(final_data_exp, propBackground > median(final_data_exp$propBackground))
low_structural_exp <- subset(final_data_exp, propBackground < median(final_data_exp$propBackground))

more_structure <- glmer(corrAns ~ numCausal + numFunctional + numSymbol + numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=high_structural_exp, nAGQ=0)
summary(more_structure)
```

```{r}
low_structure <- glmer(corrAns ~ numCausal + numFunctional + numSymbol + numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=low_structural_exp, nAGQ=0)
summary(low_structure)
```

Same for depictions. 
```{r}
## median split data by more or less structural details for depictive drawings
high_structural_dep <- subset(final_data_exp, propBackground > median(final_data_dep$propBackground))
low_structural_dep <- subset(final_data_exp, propBackground < median(final_data_dep$propBackground))

more_structure_dep <- glmer(corrAns ~ numCausal + numFunctional + numSymbol + numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=high_structural_dep, nAGQ=0)
summary(more_structure_dep)
```
```{r}
low_structure_dep <- glmer(corrAns ~ numCausal + numFunctional + numSymbol + numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=low_structural_dep, nAGQ=0)
summary(low_structure_dep)
```

Or drawings with both causal and non-causal parts, vs. drawings that only had one or the other. If there were no impact of the drawing content on part-discrimination performance, then we should see no difference between trials when making these comparisons. 
```{r}
## split by causal + functional, only causal, and only functional
only_causal <- subset(final_data, propFunctional == 0)
only_functional <- subset(final_data, propCausal == 0)

## original model with both causal + functional
m_corrAns <- glmer(corrAns ~ condition + condition*numCausal + condition*numFunctional + condition*numSymbol + condition*numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=final_data, nAGQ=0)
summary(m_corrAns)
```

```{r}
causal_check <- glmer(corrAns ~ condition + condition*numCausal + condition*numFunctional + condition*numSymbol + condition*numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=only_causal, nAGQ=0)
summary(causal_check)
```

```{r}
functional_check <- glmer(corrAns ~ condition + condition*numCausal + condition*numFunctional + condition*numSymbol + condition*numBackground + (1|sketchID) + (1|orig_gameID), family="binomial", data=only_functional, nAGQ=0)
summary(functional_check)
```
