...
orig_data <- subset(orig_data, select = -c(...1))
...
props.toys.plot <- props.toys %>% 
  ...
  facet_grid(.~toy_id) +
  geom_text(
  aes(label = sprintf("%.02f", proportion)),
  position = position_stack(vjust = .5)
  ) + 
  # geom_text(aes(label=sprintf("%.02f", proportion))) +
  ggtitle("Number of strokes") +
  xlab("condition") +
  ylab("proportion") +
  theme_light() +
...
props.toys.plot + scale_fill_manual(values = c("#7F96AC", "#63948C", "#9FC8B7", "#F4D79F"))
# ggsave('../results/plots/causaldraw_strokeType_bytoys_proportion.pdf', height = 18, width = 20, units = 'cm', useDingbats = F)
```
